...
class InputCollapser extends Collapser {
   ...
   */
  get collapsed(): boolean {
    let task = this.parent.parent as Task;
    if (task) {
      return task.inputHidden;
    } else {
      return false;
    }
  }
  ...
  protected handleClick(e: React.MouseEvent<HTMLDivElement>): void {
    let task = this.parent.parent as Task;
    if (task) {
      task.inputHidden = !task.inputHidden;
    }
...
class OutputCollapser extends Collapser {
   ...
   */
  get collapsed(): boolean {
    let task = this.parent.parent as DataintegrationTask;
    if (task) {
      return task.outputHidden;
    } else {
      return false;
    }
  }
  ...
  protected handleClick(e: React.MouseEvent<HTMLDivElement>): void {
    let task = this.parent.parent as DataintegrationTask;
    if (task) {
      task.outputHidden = !task.outputHidden;
    }
    /* We need this until we watch the cell state */
    this.update();
  }
